...
time_list = [str(d) + "小时" for d in range(0, 25)]
print(time_list)
...
def get_year_chart(year: str):
    ...
    map_chart = (
        ...
        .set_global_opts(
            title_opts=opts.TitleOpts(
                title="" + str(year) + "小时天猫双十一销售金额（单位：亿）",
                subtitle="",
                pos_left="center",
                pos_top="top",
    ...
    line_chart = (
        ...
        .set_global_opts(
            title_opts=opts.TitleOpts(
                title="天猫双十一销售金额（单位：亿）", pos_left="72%", pos_top="5%"
            )
        )
    )
...
if __name__ == "__main__":
    timeline = Timeline(
        init_opts=opts.InitOpts(width="1200px", height="700px", theme=ThemeType.DARK)
    )
    for y in time_list:
