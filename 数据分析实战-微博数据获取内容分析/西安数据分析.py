...
df1 = pd.read_csv('new_西安疫情.csv')
df1 = df1.drop(['博主','认证'],axis=1)
df2 = pd.read_csv('西安发布.csv')
df3 = pd.read_csv('西安晚报.csv')
...
def time1(x):
    ...
    x1 = str(x).split('日')
    x1 = x1[0] + '日'
    if '01月' in x1 and '年' not in x1:
        x1 = '2022年' + x1
    if "2021-" in x1 or "01-" in x1:
        x1 = x1.replace('2021-','2021年')
        x1 = x1.replace('-', '月')
        x1 = x1.split(' ')[0] + '日'
...
def main1():
    ...
    plt.savefig('./西安数据可视化/发帖热度时间趋势图.png')
    plt.show()
...
def main2():
    ...
    plt.ylabel("点赞数量")
    plt.xticks(rotation=65)
    plt.savefig('./西安数据可视化/点赞热度时间趋势图.png')
    plt.show()
...
def main3():
    ...
    plt.ylabel("时间")
    # plt.xticks(rotation=65)
    plt.savefig('./西安数据可视化/评论热度时间趋势图.png')
    plt.show()
...
def main4():
    ...
    plt.ylabel("转发数量")
    plt.xticks(rotation=65)
    plt.savefig('./西安数据可视化/转发热度时间趋势图.png')
    plt.show()
